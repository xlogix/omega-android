...
public class SubtextHelper {
  ...
  /**
   * Returns some kind of human readable representation of a past date, e.g. "4 mins ago," "2 days
   * ago"
   */
  public static String getHumanReadableDateDiff(Date pastDate, boolean unknownIfNull) {
    ...
    if (timeDiff < TimeUnit.SECONDS.toMillis(60)) { // less than a minute ago
      ...
      long hours =
          Math.round(TimeUnit.SECONDS.convert(timeDiff, TimeUnit.MILLISECONDS) / (60.0 * 60));
      if (hours == 1) {
        return "1 hour ago";
      } else {
      ...
      long days =
          Math.round(TimeUnit.SECONDS.convert(timeDiff, TimeUnit.MILLISECONDS) / (60.0 * 60 * 24));
      if (days == 1) {
        return "1 day ago";
      } else {
      ...
      long months = Math.round(
          TimeUnit.SECONDS.convert(timeDiff, TimeUnit.MILLISECONDS) / (60.0 * 60 * 24 * 30));
