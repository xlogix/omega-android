...
import xyz.fnplus.omega.util.FlagUtil;
...
public class LogAnalyzer {
  ...
  private static Pattern tryingToLaunchPattern =
      Pattern.compile("Trying to launch ([^/]++)/([^ \t}]+)");
  ...
  public static AnalyzedLogLine analyzeLogLine(String line) {
    ...
    if (startPattern.matcher(line).find()
        ...
        && !line.contains(
        "(has extras)")) {// if it has extras, we can't call it (e.g. com.android.phone)
    ...
    if (tryingToLaunchMatcher.find()) {
      ...
      return new AnalyzedLogLine(false, tryingToLaunchMatcher.group(1),
          tryingToLaunchMatcher.group(2));
    }
  ...
  private static boolean lineContainsIncompatibleFlags(String line) {
    ...
    if (flagMatcher.find()) {
      ...
      return !FlagUtil.hasFlag(flags, Intent.FLAG_ACTIVITY_NEW_TASK) || FlagUtil.hasFlag(flags,
