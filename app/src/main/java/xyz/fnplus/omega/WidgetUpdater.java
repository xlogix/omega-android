...
import xyz.fnplus.omega.db.AppHistoryDbHelper;
import xyz.fnplus.omega.db.AppHistoryEntry;
import xyz.fnplus.omega.db.SortType;
import xyz.fnplus.omega.helper.ActivityInfoHelper;
import xyz.fnplus.omega.helper.PreferenceHelper;
import xyz.fnplus.omega.helper.ResourceIdHelper;
import xyz.fnplus.omega.helper.SubtextHelper;
import xyz.fnplus.omega.util.Pair;
import xyz.fnplus.omega.util.StopWatch;
...
public class WidgetUpdater {
  ...
  private static RemoteViews buildUpdate(Context context, AppHistoryDbHelper dbHelper,
    ...
    List<Pair<AppHistoryEntry, ActivityInfo>> activityInfos =
        ActivityInfoHelper.getActivityInfos(context, dbHelper, packageManager, pageNumber,
            APPS_PER_PAGE, sortType, false);
    ...
    for (int i = 0; i < APPS_PER_PAGE; i++) {
      ...
      if (i < activityInfos.size()) {
        ...
        String label =
            ActivityInfoHelper.loadLabelFromAppHistoryEntry(context, appHistoryEntry, activityInfo,
                packageManager);

        Bitmap iconBitmap =
            ActivityInfoHelper.loadIconFromAppHistoryEntry(context, appHistoryEntry, activityInfo,
                packageManager);
        ...
        PendingIntent pendingIntent =
            PendingIntent.getActivity(context, 0 /* no requestCode */, intent, 0 /* no flags */);
  ...
  private static void setBackgroundVisibility(Context context, int appWidgetId,
      RemoteViews updateViews) {

    boolean showBackground = PreferenceHelper.getShowBackgroundPreference(context, appWidgetId);

    updateViews.setViewVisibility(R.id.widget_background,
        showBackground ? View.VISIBLE : View.INVISIBLE);
  }

  private static void setBackAndForwardButtons(Context context, int appWidgetId,
      RemoteViews updateViews, int pageNumber, AppHistoryDbHelper dbHelper, SortType sortType) {
    ...
    if (lockPage) {
      ...
      synchronized (AppHistoryDbHelper.class) {
        noMoreAppResults = dbHelper.findCountOfInstalledAppHistoryEntries(sortType, APPS_PER_PAGE,
            (pageNumber + 1) * APPS_PER_PAGE, false) == 0;
      }
      updateViews.setViewVisibility(R.id.forward_button,
          noMoreAppResults ? View.INVISIBLE : View.VISIBLE);
      ...
      updateViews.setViewVisibility(R.id.back_button,
          pageNumber == 0 ? View.INVISIBLE : View.VISIBLE);
  ...
  private static void setSubtextVisibility(Context context, int appWidgetId,
      RemoteViews updateViews, SortType sortType) {

    boolean hideSubtext = PreferenceHelper.getHideSubtextPreference(context, appWidgetId, sortType);

    int subTextVisibility =
        hideSubtext ? chooseGoneOrInvisible(context, appWidgetId) : View.VISIBLE;
  ...
  private static void setAppTitleVisibility(Context context, int appWidgetId,
    ...
    int subTextVisibility =
        hideAppTitle ? chooseGoneOrInvisible(context, appWidgetId) : View.VISIBLE;
  ...
  private static PendingIntent getPendingIntentForForwardOrBackButton(Context context,
      boolean forward, int newPageNumber, int appWidgetId) {
    ...
    Uri data =
        Uri.withAppendedPath(Uri.parse(URI_SCHEME + "://widget/id/#" + forward + newPageNumber),
            String.valueOf(appWidgetId));
    intent.setData(data);

    PendingIntent pendingIntent =
        PendingIntent.getBroadcast(context, 0 /* no requestCode */, intent,
  ...
  private static int chooseGoneOrInvisible(Context context, int appWidgetId) {
    return PreferenceHelper.getStretchToFillPreference(context, appWidgetId) ? View.GONE
        : View.INVISIBLE;
  }
}
