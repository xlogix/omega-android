...
import xyz.fnplus.omega.data.AnalyzedLogLine;
import xyz.fnplus.omega.db.AppHistoryDbHelper;
import xyz.fnplus.omega.helper.LogAnalyzer;
...
/* couldn't get this to work
  ...
  private void handleCommand(Intent intent) {
    ...
    Notification.Builder notification = new Notification.Builder(this).setTicker(tickerText)
        .setSmallIcon(R.drawable.service_notification_1)
    ...
    PendingIntent contentIntent = PendingIntent.getActivity(this, 0, appTrackerActivityIntent, 0);

    notification = notification.setContentTitle(getText(R.string.notification_title))
        .setContentText(getText(R.string.notification_subtext))
        .setContentIntent(contentIntent);

    if (PreferenceHelper.getShowNotificationPreference(getApplicationContext())) {
      startForegroundCompat(R.string.notification_title, notification.build());
    }
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (analyzedLogLine == null) {
          ...
          synchronized (AppHistoryDbHelper.class) {
            dbHelper.incrementAndUpdate(analyzedLogLine.getPackageName(),
                analyzedLogLine.getProcessName());
          }
          WidgetUpdater.updateWidget(this, dbHelper);
        }
      }
  ...
  private int getNumberOfExistingLogLines() throws IOException {
    ...
    Process logcatProcess = Runtime.getRuntime().exec(new String[] {
  ...
  private void restartAppTrackerService() {
    ...
    restartServiceIntent.setData(
        Uri.withAppendedPath(Uri.parse(AppTrackerWidgetProvider.URI_SCHEME + "://widget/restart/"),
            Long.toHexString(new Random().nextLong())));

    PendingIntent pendingIntent =
        PendingIntent.getBroadcast(this, 0 /* no requestCode */, restartServiceIntent,
            PendingIntent.FLAG_ONE_SHOT);

    AlarmManager alarms = (AlarmManager) getSystemService(Context.ALARM_SERVICE);

    long timeToExecute =
        System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(60); // start 60 seconds from now
