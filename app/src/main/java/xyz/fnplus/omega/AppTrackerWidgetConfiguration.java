...
import xyz.fnplus.omega.helper.FreemiumHelper;
...
public class AppTrackerWidgetConfiguration extends PreferenceActivity
  ...
  private void initializePreferences() {
    ...
    sortTypePreference.setSummary(String.format(getText(R.string.sort_type_summary).toString(),
        sortTypePreference.getEntry()));
    ...
    sortTypePreference.setEntries(ArrayUtil.copyOf(sortTypePreference.getEntries(),
        sortTypePreference.getEntries().length - 1));
    sortTypePreference.setEntryValues(ArrayUtil.copyOf(sortTypePreference.getEntryValues(),
        sortTypePreference.getEntryValues().length - 1));
    ...
    hideAppTitlePreference =
        (CheckBoxPreference) findPreference(R.string.hide_app_title_preference);
    ...
    stretchToFillPreference =
        (CheckBoxPreference) findPreference(R.string.stretch_to_fill_preference);
  ...
  private void sendOutBroadcast() {
    ...
    widgetUpdate.setData(
        Uri.withAppendedPath(Uri.parse(AppTrackerWidgetProvider.URI_SCHEME + "://widget/id/"),
  ...
  private void saveConfigurations() {
    ...
    PreferenceHelper.setSortTypePreference(getApplicationContext(), sortType.toString(),
        appWidgetId);
    ...
    int pageNumber =
        pageNumberEntry != null ? (Integer.parseInt(pageNumberEntry.toString()) - 1) : 0;
    ...
    PreferenceHelper.setStretchToFillPreference(getApplicationContext(), stretchToFill,
        appWidgetId);
  }
  ...
  private void updateSortTypePreference(Preference preference, Object newValue) {
    ...
    sortTypePreference.setSummary(
        String.format(getText(R.string.sort_type_summary).toString(), newValueAsEntry));
  }

  private void updateNonSortTypePreference(Preference preference, Object newValue) {
    ...
    if (!(Boolean) newValue) {
      if (preference.getKey().equals(lockPagePreferenceKey)) {
        enableStretchToFill =
            hideAppTitlePreference.isChecked() || hideSubtextPreference.isChecked();
